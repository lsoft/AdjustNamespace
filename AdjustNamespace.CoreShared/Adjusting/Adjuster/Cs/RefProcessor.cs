...
namespace AdjustNamespace.Adjusting.Adjuster.Cs
{
    ...
    public readonly struct RefProcessor
    {
            ...
            )
        {
            ...
            AdjustLog.WriteLine($"[Adjust] RefProcessor: searching references of {symbolInfo.ToDisplayString()} (target namespace {_targetNamespaceInfo.ModifiedName})");

            var foundReferences = await FindReferencesForAsync(_workspace, symbolInfo, cancellationToken);

            AdjustLog.WriteLine($"[Adjust] RefProcessor: {symbolInfo.ToDisplayString()} -> {foundReferences.Count} referenced symbol(s)");
            ...
            foreach (var foundReference in foundReferences)
            {
                ...
                {
                    AdjustLog.WriteLine($"[Adjust] RefProcessor: {foundReference.Definition.ToDisplayString()} already in target namespace, {locationCount} location(s) skipped");

                    //referenced symbols is in target namespace already
                    continue;
                }

                AdjustLog.WriteLine($"[Adjust] RefProcessor: {foundReference.Definition.ToDisplayString()} ({foundReference.Definition.Kind}) has {locationCount} location(s)");
            ...
            )
        {
            ...
            if (root == null)
            {
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}@{location.Location.SourceSpan}: no syntax root, skipped");
            ...
            if (syntax == null)
            {
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}@{location.Location.SourceSpan}: FindNode returned null, skipped");
            ...
            {
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}@{syntax.Span}: no semantic model, skipped");
            ...
            if (symbol == null)
            {
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}@{syntax.Span} ('{syntax}'): GetSymbolInfo returned null, skipped");
            ...
            if (qcs != null)
            {
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}@{syntax.Span}: '{syntax}' is the cref '{qcs}'");
            ...
            else if (syntax.Parent is MemberAccessExpressionSyntax maes)
            {
                ...
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}@{syntax.Span}: '{syntax}' is a part of the member access '{maesr}' (symbol kind {symbol.Kind})");
            ...
            else
            {
                ...
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}@{syntax.Span}: '{syntax}' has an unrecognized parent ({syntax.Parent?.GetType().Name})");
            ...
            )
        {
            ...
            AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}: replace '{uqns}' with '{mqns}'");
            ...
            )
        {
            ...
            {
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}: cref container '{qcs.Container}' is not a namespace -> add using {_targetNamespaceInfo.ModifiedName}");
            ...
            AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}: replace cref container '{qcs.Container}' with '{modified}'");
            ...
            )
        {
            ...
            {
                AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}: '{maes}' member is {symbol.Kind} -> add using {_targetNamespaceInfo.ModifiedName}");
            ...
            AdjustLog.WriteLine($"[Adjust] RefProcessor: {location.Document.FilePath}: replace '{maes}' with '{modifiedMaesr}' (typeIndex={typeIndex})");
            ...
            )
        {
            ...
            AdjustLog.WriteLine($"[Adjust] RefProcessor: SymbolFinder.FindReferencesAsync({symbolInfo.ToDisplayString()}) -> {foundReferences.Count} symbol(s)");
