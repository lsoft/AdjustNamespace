...
namespace AdjustNamespace.Namespace
{
    ...
    public class NamespaceCenter
    {
        ...
        /// as soon as a single one of them still has a type declared directly in that
        /// namespace (child namespaces do not count: <c>using Parent;</c> does not import them).
        /// </summary>
            ...
            )
        {
            ...
            foreach (var compilation in compilations)
            {
                ...
                if (@namespace != null && @namespace.GetDirectTypes().Any())
                {
                    AdjustLog.WriteLine($"[Adjust] NamespaceCenter: {namespaceName} is still filled for {compilation.AssemblyName}, using clause kept");

                    return false;
                }
            }

            AdjustLog.WriteLine($"[Adjust] NamespaceCenter: {namespaceName} is gone for {string.Join(", ", compilations.Select(c => c.AssemblyName))}, using clause removed");
        ...
        public void TypeRemoved(ITypeSymbol type)
        {
            ...
            if (set.Count == 0)
            {
                AdjustLog.WriteLine($"[Adjust] NamespaceCenter: {cnn} is now empty, its using clauses will be removed");
        ...
        public void TypeAdded(ITypeSymbol type, string targetNamespace)
        {
            ...
            {
                AdjustLog.WriteLine($"[Adjust] NamespaceCenter: {targetNamespace} is filled again, its using clauses will be kept");
            }
        }
